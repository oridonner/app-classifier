...
class AppClassifier(object):
    ...
    def predict_segment(self,target_data_dict,path):
        self._predict_is_supported(target_data_dict)
        self.tf['word_count']=gl.text_analytics.count_words(self.tf['description'],to_lower=True)
        self.tf['tf_idf']=gl.text_analytics.tf_idf(self.tf['word_count'])
        self.tf['segment'] = self.segment_classifier.predict(self.tf)
        self.tf.remove_column('word_count')
        self.tf.remove_column('tf_idf')
        self.tf.save(path, format='csv')
        


    # Predict if app is supported (if it's description language is english), using the english classifier model
    def _predict_is_supported(self,target_data_dict):
        self.tf = self._load_data(target_data_dict)
        self._add_helper_columns(self.tf)
        self.tf['is_supported'] = self.english_classifier.predict(self.tf)
    ...
    def _build_segment_classifier(self):
        self.sf['word_count']=gl.text_analytics.count_words(self.sf['description'],to_lower=True)
        self.sf['tf_idf']=gl.text_analytics.tf_idf(self.sf['word_count'])
        train_data,test_data=self.sf[self.sf['is_supported']].random_split(.8,seed=0)
        segment_classifier = gl.classifier.create(train_data, target='segment', features=['tf_idf'], validation_set=test_data)
        self.segment_classifier = segment_classifier
        self.sf.remove_column('word_count')
        self.sf.remove_column('tf_idf')
